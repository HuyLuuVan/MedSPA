...
TRAIN_SCRIPT="${TRAIN_SCRIPT:-grpo_summary_paper_aligned.py}"
...
DATASET_SUMMARY="${DATASET_SUMMARY:-${REPO_ROOT}/data/summary_data_grpo.json}"


CORE_NAME="${CORE_NAME:-medspa_grpo_summary}"
...
OUTPUT_DIR="${OUTPUT_DIR:-${REPO_ROOT}/checkpoints/grpo/summary/${RUN_NAME}}"

mkdir -p "$OUTPUT_DIR"

LOGFILE="${OUTPUT_DIR}/train_summary.log"
...
export CHEXBERT_CKPT="${CHEXBERT_CKPT:-${REPO_ROOT}/dataset/chexbert_ckpt/chexbert.pth}"
...
if [[ ! -f "${WORKDIR}/${TRAIN_SCRIPT}" ]]; then
    echo "[ERROR] Training script not found: ${WORKDIR}/${TRAIN_SCRIPT}"
    exit 1
fi

if [[ ! -f "$DATASET_SUMMARY" ]]; then
    echo "[ERROR] Dataset not found: ${DATASET_SUMMARY}"
    exit 1
fi
...
if [[ ! -f "$CHEXBERT_CKPT" ]]; then
    echo "[ERROR] CheXbert checkpoint not found."
    exit 1
fi


echo "[INFO] Starting MedSPA SUM R-Align training."
...
if [[ "$EXIT_CODE" -eq 0 ]]; then
    echo "[INFO] GRPO summary training completed successfully."
else
    echo "[ERROR] GRPO summary training failed with exit code ${EXIT_CODE}."
fi

echo "[INFO] Training log: ${LOGFILE}"
