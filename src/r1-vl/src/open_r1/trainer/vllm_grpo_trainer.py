...
try:
    # Qwen2-VL config class (có thể có/không tùy phiên bản transformers)
    from transformers.models.qwen2_vl import Qwen2VLConfig  # type: ignore
except Exception:
    Qwen2VLConfig = None  # fallback nếu import không được


def _is_vision_lang_config(cfg) -> bool:
    """
    Phát hiện các model đa phương thức để dùng AutoModelForVision2Seq.
    Ưu tiên kiểm tra kiểu class (Qwen2VLConfig), nếu không thì fallback theo heuristics.
    """
    # 1) nếu có class Qwen2VLConfig và isinstance khớp -> chắc chắn là VL
    if Qwen2VLConfig is not None and isinstance(cfg, Qwen2VLConfig):
        return True

    # 2) kiểm tra model_type an toàn với cả gạch ngang/underscore, lower-case
    mt = (getattr(cfg, "model_type", "") or "").lower().replace("-", "_")
    if mt in {
        "qwen2_vl", "mllama", "phi4_multimodal", "git", "fuyu",
        "llava", "llava_next", "llava_onevision", "idefics2", "emu3",
    }:
        return True

    # 3) heuristics: có vision_config hoặc image_embed_dim… → nhiều khả năng là VL
...
def load_auto_for_policy(model_name_or_path: str, **kwargs):
    """
    Chọn đúng AutoModel theo config:
      - Vision-Language → AutoModelForVision2Seq
      - Còn lại → AutoModelForCausalLM
    """
    # đảm bảo trust_remote_code mặc định True cho Qwen2-VL
    ...
    def __init__(
        ...
        if isinstance(model, str):
            ...
            if "Qwen2-VL" in model_id:
                ...
                model = load_auto_for_policy(model, **model_init_kwargs)
        else:
