...
PY_SCRIPT="${PY_SCRIPT:-${REPO_ROOT}/src/r1-vl/infer/infer_reason/infer_sft_reason_clean.py}"

BASE_MODEL="${BASE_MODEL:-Qwen/Qwen3-VL-8B-Instruct}"
LORA_PATH="${LORA_PATH:-${REPO_ROOT}/checkpoints/sft_reason}"

IU_SHARD_DIR="${IU_SHARD_DIR:-${REPO_ROOT}/data/iu/test_shards}"

ALL_IMAGES_PATH="${ALL_IMAGES_PATH:-${REPO_ROOT}/iu_cxr_all_images.txt}"
FINDINGS_JSON="${FINDINGS_JSON:-${REPO_ROOT}/iu_cxr_findings.json}"

OUTPUT_DIR="${OUTPUT_DIR:-${REPO_ROOT}/results/iu/sft_reason}"

PREFIX_IN="${PREFIX_IN:-annotation_test_shard}"
...
if [[ "$PRETRAINED_ONLY" != "1" && \
      "$PRETRAINED_ONLY" != "true" && \
      "$PRETRAINED_ONLY" != "TRUE" ]]; then
...
if [[ ! -d "$IU_SHARD_DIR" ]]; then
    echo "[ERROR] IU shard directory not found: $IU_SHARD_DIR"
    exit 1
fi
...
if [[ ! -f "$FINDINGS_JSON" ]]; then
    echo "[ERROR] IU findings file not found: $FINDINGS_JSON"
    exit 1
fi
...
for ((WAVE_START=0; WAVE_START<NUM_SHARDS; WAVE_START+=NUM_GPUS)); do
    ...
    for ((GPU_ID=0; GPU_ID<NUM_GPUS; GPU_ID++)); do
        ...
        SHARD_JSON="${IU_SHARD_DIR}/${BASE_IN}${SUFFIX_IN}"
...
echo
echo "======================================"
echo "[INFO] IU SFT PR inference finished."
